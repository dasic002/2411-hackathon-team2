...
This logo captures the site's focus on accessibility and sharing information in a way that is easy for everyone to engage with. The logo is also used as the favicon.

#### Navigation Menu

The navigation bar is very intuitive and easy to navigate through the different options of HOME - CONTACT US - FAQ.  When viewed on mobile the menu turns into a burger icon and expands to a full menu on tablets and desktops. 

#### Hero Section

In the Hero section we can see displayed the chosen title of the app "NEWS ECHO" to enhance its purpose. Depending on the time of the day, the user is greeted by different messages: "Good Morning", "Good Afternoon" and "Good Evening". The user will be also able to check the current date depending on the user's timezone.

![Hero Screenshot](assets/media/readme/hero_picture.png)

#### Weather Widget

The weather widget is displayed clearly, using a universal cloud symbol.  Todays current weather is displayed with a large volume speak button, when the user selects this the weather will be spoken out loud to them.  If they click and select anywhere on the weather it will display weather for the next few days. It is intuitive and easy to use. It has been designed with users who may have poor motor skills and or visual impairments. 

#### News Widget

The news widget is a large News symbol button with sound waves, when the user clicks on the large speaker button the news will be spoken to them.  They can choose to select to have the News headline title spoken to them first and they choose weather they wish to listen to the full article.  This allows them to choose between different news articles that they are interested in.

#### Footer

The Footer is divided in two parts. In the first is displayed a "Contact us" button that brings the user to the Team page.
In the second part, the user will be able to access social media pages from an Irish association called "Fighting Blindness" for support or information.
...
<table>
<tr>
      <th>Colour Contrast Combinations</th>
  </tr>
  <tr>
...
Our final chosen colour scheme was **Colour Scheme 1 (Classic Contrast)**, based on the results and a vote amongst our team members.
...
* Create colour-blind friendly themes
...
#### Viki
Viki did the FAQ page and assisted with the Readme documentation and thoroughly enjoyed the whole hackathon and learned many new things! 
